...
TW.toggleAxes = function (mode) {
    ...
    if( mode === "show" ) {
        ...
        state.axisHelper.visible = false;
    } else {
        // this is the normal usage
        state.axisHelper.visible = ! state.axisHelper.visible;
    }
    state.render();
};
...
// Adds an AxisHelper with given dimensions to the scene. It's initially
// invisible.

TW.addAxisHelper = function (scene, lineLength, lineWidth ) {
    var state = TW.lastClickTarget.TW_state;
    var ah = new THREE.AxisHelper( lineLength );
...
TW.mainInit = function(renderer,scene,inits) {
    ...
    TW.addAxisHelper(scene, axisLength, 5);
...
TW.cameraSetup = function (renderer,scene,scene_bb,inits) {
    ...
    if( ah == null ) {
        throw("Your scene has no AxisHelper, which means it wasn't created by "
                    +"TW.mainInit()." );
    }
...
THREE.BezierSurfaceGeometry.prototype.computeBezierSurfacePoints =
    ...
    function bez(s,t) {
        ...
        // see, for example, http://en.wikipedia.org/wiki/Bézier_surface
        var i, j, M = 3, N = 3;
...
TW.showShaders = function (sceneObject) {
    if( sceneObject instanceof  THREE.Mesh ) {
        ...
        vspre.innerHTML = mat.vertexShader;
        var fspre = document.createElement('PRE');
        fspre.innerHTML = mat.fragmentShader;
        var vsp = document.createElement('P');
        vsp.innerHTML = "The object's vertex shader:";
        var fsp = document.createElement('P');
        fsp.innerHTML = "The object's fragment shader:";
